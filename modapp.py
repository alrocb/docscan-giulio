...
# Check for OpenAI API key in environment or secrets
...
if not openai_api_key:
    st.error("⚠️ OpenAI API key is missing! Please add your API key to deploy this app.")
    st.info("You can add your API key in the Streamlit Cloud dashboard under 'Manage app' > 'Secrets'")
    st.stop()
...
def extract_metadata_from_path(folder_path: str) -> Dict[str, str]:
    ...
    Handles specific folder names like 'VARIOUS' for doc_type.
    """
    metadata = {}
    ...
    if sf_match:
        ...
        # Handle cases like SF_S3161 or just SF_
        if sf_code and sf_code != '_': 
            metadata['sf_code'] = sf_code
        else:
            metadata['sf_code'] = "unknown"  # Default value for empty or placeholder SF code
    ...
    # Handle specific known folder names if the pattern doesn't match
    elif folder_name.upper() == "VARIOUS":
         metadata['doc_type'] = "various"
    # Add other specific folder names here if needed
    # elif folder_name.upper() == "SHIPPING_DOCS":
    #     metadata['doc_type'] = "shipping docs"
    else:
        # Fallback: use the folder name itself (cleaned) if no pattern matches
        metadata['doc_type'] = folder_name.lower().replace('_', ' ')
...
def extract_zip_and_process(zip_file):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        # First try the temp directory directly
        docs = process_folder_structure(tmpdirname)
        
        if not docs:
            # If no documents found, look for subdirectories that might contain the PO folders
            # This handles cases where the ZIP file contains a root folder
            for item in os.listdir(tmpdirname):
                subdir_path = os.path.join(tmpdirname, item)
                if os.path.isdir(subdir_path):
                    subdocs = process_folder_structure(subdir_path)
...
A delivery can include one or more packages (or boxes). For each package, extract the following details:
- PO/TR No.
- SF No.
- WAYBILL N° (B/L/AWB): CRITICALLY IMPORTANT - This is the shipping tracking number used for tracking packages. 
  * For air shipments, extract the Air Waybill (AWB) number. Look for prefixes like "AWB:", "Air Waybill:", "Waybill No:", "MAWB", etc.
  * For sea shipments, extract the Bill of Lading (B/L) number. Look for prefixes like "B/L:", "Bill of Lading:", "MBL No:", etc.
  * This information is PRIMARILY found in documents from the 'VARIOUS' folder or in shipping-specific documents.
  * AWB numbers are typically 11-12 digits (may include carrier prefix).
  * B/L numbers typically include letters and numbers in specific formats (e.g., MAEU1234567).
  * If you cannot find any legitimate AWB or B/L number after thorough search, ONLY then output "N.A."
...
Note: Some fields (like PO/TR No., SF No., WAYBILL, and VENDOR) may be common for all packages within the delivery.
...
Output the result as a JSON array. If no packages are found, output an empty JSON array [].
    ...
    if extract_button:
        if not st.session_state.documents_for_extraction:
            ...
            with st.spinner("Extracting information from documents..."):
                ...
                # Group documents by PO/SF combination for context-aware processing
                for doc in st.session_state.documents_for_extraction:
                    po_number = doc.metadata.get('po_number', 'unknown')
                    sf_code = doc.metadata.get('sf_code', 'unknown')
                    
                    # Group by PO and SF only to process all docs for a delivery together
                    key = f"PO_{po_number}_SF_{sf_code}" 
                ...
                st.write(f"Processing {len(metadata_groups)} delivery groups (PO/SF combinations)...")
                
                # Process each group (each unique PO/SF combination)
                for i, (key, docs) in enumerate(metadata_groups.items()):
                    # Update progress
                    progress = (i + 1) / len(metadata_groups)
                    progress_bar.progress(progress)
                    progress_text.text(f"Processing delivery {i+1}/{len(metadata_groups)}: {key}")
                    
                    # Get metadata from the first document in group (PO/SF should be consistent)
                    metadata = docs[0].metadata 
                    
                    # Show delivery being processed
                    st.write(f"Processing delivery group: {key}")
                    
                    # Combine text from all docs in this group (all types for this PO/SF)
                    combined_text = "\n\n--- Document Separator ---\n\n".join([f"Source: {doc.metadata.get('source_file', 'N/A')}\nType: {doc.metadata.get('doc_type', 'N/A')}\n\n{doc.page_content}" for doc in docs])
                    
                    # If text is too long, trim it (consider smarter trimming if needed)
                    MAX_CHARS = 15000 # Increased slightly, adjust as needed based on model context window
                    if len(combined_text) > MAX_CHARS:
                        st.write(f"⚠️ Combined text for {key} is very long ({len(combined_text)} chars), trimming to first {MAX_CHARS} chars")
                        combined_text = combined_text[:MAX_CHARS]
                    
                    # Run extraction using the updated extraction_template
                    try:
                        ...
                        # Handle potential empty or non-JSON results gracefully
                        if not extraction_result_clean or extraction_result_clean == "[]":
                             st.write(f"No package information extracted for {key}.")
                             continue # Skip to the next group

                        packages = json.loads(extraction_result_clean)
                        if isinstance(packages, dict): # Handle case where LLM returns a single object instead of array
                            packages = [packages]
                        elif not isinstance(packages, list): # Ensure it's a list
                            st.error(f"Unexpected format from extraction for {key}. Expected JSON array.")
                            st.code(extraction_result_clean)
                            continue

                        # Add metadata to each package
                        for pkg in packages:
                            # Ensure pkg is a dictionary before proceeding
                            if not isinstance(pkg, dict):
                                st.warning(f"Skipping invalid package entry in results for {key}: {pkg}")
                                continue

                            # Use extracted metadata from folder structure if available and not already filled by LLM
                            if 'po_number' in metadata and not pkg.get("PO_TR_No"):
                                pkg["PO_TR_No"] = metadata['po_number']
                            if 'sf_code' in metadata and not pkg.get("SF_No"):
                                pkg["SF_No"] = metadata['sf_code']
                            
                            # Add source file from the first doc in the group for reference
                ...
                if all_packages:
                    st.success(f"Successfully extracted information for {len(all_packages)} packages across {len(metadata_groups)} deliveries")
                    ...
                    desired_columns = {
                        ...
                        "SOURCE_FILE": "Source File (First Doc)"
                    }
                    
                    # Select and rename columns present in the DataFrame
                    rename_mapping = {k: v for k, v in desired_columns.items() if k in df.columns}
                    df = df[list(rename_mapping.keys())] # Reorder and select columns
                    df.rename(columns=rename_mapping, inplace=True)
                    
                    # Combine with existing data if it exists
                    if existing_df is not None:
                        # Ensure columns match before combining, handle missing columns
                        existing_cols = set(existing_df.columns)
                        new_cols = set(df.columns)
                        
                        # Add missing columns to both dataframes, filled with None or pd.NA
                        for col in new_cols - existing_cols:
                            existing_df[col] = pd.NA
                        for col in existing_cols - new_cols:
                            df[col] = pd.NA
                            
                        # Reorder columns to match before concat
                        df = df[existing_df.columns]

                        # Check for duplicates based on key columns if they exist
                        duplicate_columns = ["PO/TR No.", "SF No.", "WAYBILL N° (B/L/AWB)"]
                        
                        try:
                            # Attempt to drop duplicates based on a subset of columns if they exist
                            key_cols_exist_in_existing = all(col in existing_df.columns for col in duplicate_columns)
                            key_cols_exist_in_new = all(col in df.columns for col in duplicate_columns)

                            if key_cols_exist_in_existing and key_cols_exist_in_new:
                                # Create unique identifiers for rows in both dataframes
                                existing_ids = existing_df[duplicate_columns].astype(str).agg('_'.join, axis=1)
                                new_ids = df[duplicate_columns].astype(str).agg('_'.join, axis=1)
                                
                                # Keep only rows from df that don't have a matching ID in existing_df
                                new_rows_mask = ~new_ids.isin(existing_ids)
                                new_rows = df[new_rows_mask].copy()
                                
                                combined_df = pd.concat([existing_df, new_rows], ignore_index=True)
                                st.success(f"Combined data. Added {len(new_rows)} new unique entries.")
                            else:
                                # Fallback to simple concatenation if key columns aren't fully present
                                combined_df = pd.concat([existing_df, df], ignore_index=True)
                                st.warning("Could not perform precise duplicate check due to missing key columns. Appended all new extractions.")
                                
                        except Exception as e:
                             st.error(f"Error during duplicate check/merge: {e}. Concatenating all data.")
                             combined_df = pd.concat([existing_df, df], ignore_index=True)
